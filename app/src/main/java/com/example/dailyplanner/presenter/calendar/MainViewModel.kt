...
class MainViewModel @Inject constructor (private val getListCaseUseCase: GetListCaseUseCase,private val addDayCaseUseCase: AddDayCaseUseCase,private val userUseCase: UserUseCase): ViewModel() {
    ...
    fun getDayCases(year:Int,month:Int,day:Int){
        viewModelScope.launch {
            ...
            try {
                ...
                _state.value = State.Succses(listCases, userUseCase.getUserLogIn()!!)
            }catch (e:Exception){
    ...
    fun out() {
        viewModelScope.launch {
            userUseCase.deleteUserLogIn()
            _state.value = State.SuccsesOut
        }
    }
